...
class ThumbNailViewController: UITableViewController {
    ...
    func scrollingToTop(_sender: UITapGestureRecognizer) {
        let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(getter: self.thumbNailTableView))
        tapGestureRecognizer.numberOfTapsRequired = 1
        thumbNailTableView.addGestureRecognizer(tapGestureRecognizer)
        thumbNailTableView.isUserInteractionEnabled = true
    
        let topOffset = CGPoint(x: 0, y: -(self.tableView?.contentInset.top ?? 0))
